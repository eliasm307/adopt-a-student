...
import {
  createLocaleSubjectDocumentId as createLocaleSubjectDocumentId, isLocaleSubjectDocumentId,
...
> = async (props) => {
  ...
  if (!isLocaleSubjectDocumentId(id))
  ...
  const localeSubject = await getDocumentData({
    ...
    documentId: createLocaleSubjectDocumentId({
      ...
      genericId: id,
      locale,
    }), // todo see if any other locale subject queries need to use this function instead of trying to use the raw id
